...
export default function MovieDetails({ selectedMovie, primaryButton, closeButton }: MovieDetailsProps) {
  return (
    <div>
      <div
        className="w-full h-52 bg-cover bg-top mb-2 rounded-md"
        style={{
          backgroundImage: `url(${selectedMovie?.backdropUrl})`,
        }}
      >
      </div>
      <h2 className="text-3xl font-bold">{selectedMovie.title} <span className="font-light">({selectedMovie.year})</span></h2>
